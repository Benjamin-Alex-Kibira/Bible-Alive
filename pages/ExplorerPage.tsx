...
const ExplorerPage: React.FC = () => {
  ...
  const handleSearch = async (book: string, chapter: number, verseNum: number) => {
    ...
    try {
      const result = await getVerse(book, chapter, verseNum);
      if (result) {
        setVerse(result);
      } else {
  ...
  const createGenerationPrompt = (verse: BibleVerse): string => {
    return `Visualize the following scripture from ${verse.book} ${verse.chapter}:${verse.verse}: "${verse.text}". The depiction must be historically and culturally accurate to the time period, deeply reverent, and faithful to the literal meaning found in the original Hebrew, Aramaic, or Greek texts. Avoid modern artistic interpretations, anachronisms, or theological symbolism not explicitly present in the text. The goal is a pure, powerful visualization of the scripture as it was written. Style: Cinematic, Epic, Photorealistic.`;
  };
  ...
  const pollVideoOperation = useCallback(async (operation: GenerateVideosOperation) => {
    ...
    if (op.done) {
      ...
      if (downloadLink) {
         // In a real app with auth, you'd fetch this securely. 
         // For the demo, we append the API key if not in mock mode.
      ...
      // FIX: Safely access progressPercent and ensure it is a number before calling toFixed.
  ...
  return (
    <div>
      ...
      <ScriptureSearchForm onSearch={handleSearch} disabled={isLoading || isGeneratingVideo || isGeneratingImage} />

      <div className="mt-12 max-w-4xl mx-auto">
        {isLoading && <Spinner />}
        {error && <p className="text-center text-red-400 bg-red-900/20 p-4 rounded-lg">{error}</p>}
        {verse && (
          <div className="bg-deep-indigo/50 p-6 md:p-8 rounded-lg shadow-lg border border-gold-accent/20">
            <h2 className="text-2xl font-serif text-gold-accent">{`${verse.book} ${verse.chapter}:${verse.verse}`}</h2>
        ...
        <div className="mt-8">
            ...
            {generatedVideo && (
                 <div className="bg-black/20 p-4 rounded-lg mt-8">
